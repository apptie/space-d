...
class WordExceptionControllerTest extends CommonExceptionControllerTest {

    @Test
    void exceptions() throws Exception {
        ResultActions resultActions = mockMvc.perform(
                get("/test/words/exceptions").contentType(MediaType.APPLICATION_JSON)
        );

        MvcResult mvcResult = resultActions.andReturn();
        WordExceptionDocs data = findExceptionData(mvcResult, WordExceptionDocs.class);
                ...
                .andDo(
                        restDocs.document(
                                customResponseFields(
                                        ...
                                        exceptionConvertFieldDescriptor(data.readWordException())
                                )
                        )
                );
    }
