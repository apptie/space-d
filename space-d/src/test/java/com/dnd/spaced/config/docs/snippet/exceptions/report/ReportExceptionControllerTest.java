...
class ReportExceptionControllerTest extends CommonExceptionControllerTest {

    @Test
    void exceptions() throws Exception {
        ResultActions resultActions = mockMvc.perform(
                get("/test/reports/exceptions").contentType(MediaType.APPLICATION_JSON)
        );

        MvcResult mvcResult = resultActions.andReturn();
        ReportExceptionDocs data = findExceptionData(mvcResult, ReportExceptionDocs.class);
                ...
                .andDo(
                        restDocs.document(
                                customResponseFields(
                                        ...
                                        exceptionConvertFieldDescriptor(data.reportException())
                                )
                        )
                );
    }
