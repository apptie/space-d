...
public enum Experience {
    ...
    private static final String EXCEPTION_FORMAT = "잘못된 경력 '%s'을(를) 입력했습니다.";
    ...
    public static Experience findBy(String name) {
        return Arrays.stream(Experience.values())
                     .filter(experience -> experience.name.equals(name))
                     .findAny()
                     .orElseThrow(() -> new InvalidExperienceException(String.format(EXCEPTION_FORMAT, name)));
    }
}
