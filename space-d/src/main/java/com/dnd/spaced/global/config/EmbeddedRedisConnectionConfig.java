...
@Profile("local || test")
@Configuration
public class EmbeddedRedisConnectionConfig {
    ...
    public void startRedis() throws IOException {
        targetPort = isRedisRunning() ? findAvailablePort() : redisPort;
    ...
    public RedissonClient redissonClient() {
        ...
        config.useSingleServer()
              .setAddress("redis://localhost:" + targetPort);
    ...
    private RedisServer initRedisServer() {
        if (isArmMac()) {
            return new RedisServer(getRedisFileForArcMac(), targetPort);
        }

        return new RedisServer(targetPort);
    }
}
